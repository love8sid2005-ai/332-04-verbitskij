...
class PhoneValidationTest extends TestCase
{
    ...
    /**
     * Тест: валидный формат 8-XXX-XXX-XX-XX
     */
    public function testValidPhoneFormat8(): void
    {
        $phone = '8-999-123-45-67';
        $this->assertTrue(validatePhone($phone), 'Формат 8-XXX-XXX-XX-XX должен быть валидным');
    }
    
    /**
     * Тест: валидный формат +79991234567
     */
    public function testValidPhoneFormatPlus7NoDashes(): void
    {
        $phone = '+79991234567';
        $this->assertTrue(validatePhone($phone), 'Формат +79991234567 должен быть валидным');
    }
    
    /**
     * Тест: валидный формат 89991234567
     */
    public function testValidPhoneFormat8NoDashes(): void
    {
        $phone = '89991234567';
        $this->assertTrue(validatePhone($phone), 'Формат 89991234567 должен быть валидным');
    }
    ...
    /**
     * Тест: невалидный номер - без плюса (для формата +7-XXX-XXX-XX-XX)
     */
    public function testInvalidPhoneWithoutPlus(): void
    {
        $phone = '7-999-123-45-67';
        $this->assertFalse(validatePhone($phone), 'Номер 7-XXX-XXX-XX-XX без + должен быть невалидным');
    }
    ...
    /**
     * Тест: невалидный номер - слишком короткий (123)
     */
    public function testInvalidPhoneVeryShort(): void
    {
        $phone = '123';
        $this->assertFalse(validatePhone($phone), 'Номер 123 должен быть невалидным');
        $normalized = normalizePhone($phone);
        $this->assertFalse($normalized, 'Номер 123 не должен нормализоваться');
    }
}

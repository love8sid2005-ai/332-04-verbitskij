...
function runLoginTests($runner) {
    ...
    $runner->runTest("Вход: Успешная авторизация", function() use ($createUser) {
        ...
        assert($result["success"] === true, "Вход с верными данными должен быть успешным");
    });
    ...
    $runner->runTest("Вход: Сессия создается при успешном входе", function() use ($createUser) {
        $createUser();
        loginUser("+79221110500", "Test123");
        assert(isset($_SESSION["user_id"]), "После входа должна быть переменная user_id в сессии");
        assert(!empty($_SESSION["user_id"]), "user_id не должна быть пустой");
    });
    ...
    $runner->runTest("Вход: Номер телефона в сессии", function() use ($createUser) {
        $createUser();
        loginUser("+79221110500", "Test123");
        assert($_SESSION["phone"] === "+79221110500", "Номер телефона должен быть в сессии");
    });
    ...
    $runner->runTest("Вход: checkAuth возвращает true после входа", function() use ($createUser) {
        $createUser();
        loginUser("+79221110500", "Test123");
        assert(checkAuth() === true, "checkAuth должна возвращать true после входа");
    });
}
?>
